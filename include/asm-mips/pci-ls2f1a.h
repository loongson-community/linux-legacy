					...
					size_t size, int direction)
{
	...
	daddr = swiotlb_map_single(hwdev, ptr, size, direction);

	addr = bus_to_virt(baddr_to_bus(hwdev->bus, daddr));
				    ...
				    size_t size, int direction)
{
	...
	return swiotlb_unmap_single(hwdev, dma_addr, size, direction);
}
                                      ...
                                      int direction)
{
	...
	return pci_map_single(hwdev, addr, size, direction);
}
			     ...
			     int nents, int direction)
{
	...
	swiotlb_map_sg(hwdev, sg, nents, direction);
	...
	for (i = 0; i < nents; i++, sg++) {
		if (sg->address && sg->page)
			...
			dma_cache_wback_inv( (unsigned long)bus_to_virt(baddr_to_bus(hwdev->bus, sg->dma_address)),
			                    sg->length);
	}
				...
				int nents, int direction)
{
	...
	for (i = 0; i < nents; i++, sg++) {
		...
		dma_cache_wback_inv((unsigned long)bus_to_virt(baddr_to_bus(hwdev->bus, sg->dma_address)), sg->length);
	}
	
	swiotlb_unmap_sg(hwdev, sg0, nents, direction);
}
				       ...
				       size_t size, int direction)
{
	...
	dma_cache_wback_inv(addr, size);
	swiotlb_sync_single(hwdev, dma_handle, size, direction);
}
				   ...
				   int nelems, int direction)
{
	...
	for (i = 0; i < nelems; i++, sg++)
		dma_cache_wback_inv((unsigned long)bus_to_virt(baddr_to_bus(hwdev->bus, sg->dma_address)), sg->length);
#endif
	swiotlb_sync_sg(hwdev, sg, nelems, direction);
}

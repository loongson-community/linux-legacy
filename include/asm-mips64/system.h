...
struct rt_hal {	   
   ...
   unsigned int (*mips_timer_interrupt)(int irq, struct pt_regs *regs); //8
   unsigned int (*mips_interrupt)(int irq, struct pt_regs *regs); //16
   long long (*rtai_srq_interrupt)(unsigned int srq, unsigned int args); //24
   void (*disint)(void); //32
   void (*enint)(void); //40
   unsigned int rtai_active; //48
   unsigned long (*getflags)(void); //56
   void (*setflags)(unsigned long flags); //64
   unsigned long (*getflags_and_cli)(void); //72
   void *irq_desc; //80
   union { unsigned long tsc; unsigned int hltsc[2]; } tsc; //88
   void (*linux_mips_timer_intr)(int irq, void *dev_id, struct pt_regs *regs); //96
   void (*linux_soft_mips_timer_intr)(int irq, void *dev_id, struct pt_regs *regs); //104
   unsigned long (*getflags_and_sti)(void); //112
   void (*soft_enint)(void); //120
};

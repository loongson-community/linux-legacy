...
static int rd_blkdev_pagecache_IO(int rw, struct buffer_head * sbh, int minor)
{
	...
	if(!(minor> 1))
          mapping = rd_bdev[2]->bd_inode->i_mapping;
	else
#endif
...
static int __init rd_init (void)
{
	...
	rd_bdev[2] = bdget(kdev_t_to_nr(MKDEV(MAJOR_NR, 2)));
	rd_bdev[2]->bd_openers++;
	rd_bdev[2]->bd_inode->i_mapping->gfp_mask = GFP_HIGHUSER;

...
#define dev_err(dev,fmt...) printk(KERN_ERR fmt)

#define CONFIG_MACH_LS1A 1
#define DMA_ACCESS_ADDR     0x1fe78040
#define ORDER_REG_ADDR      (KSEG1ADDR(0x12d01160))
...
static int ls1a_nand_init_buff(struct ls1a_nand_info *info)
{
    	...
    	info->data_buff = pci_alloc_consistent(pdev, MAX_BUFF_SIZE,
				&info->data_buff_phys);
...
{
    return(*((volatile unsigned int*)_NAND_BASE) & (0x1<<10));
}
...
int status_time;
    ...
    if((dma_base->cmd)&(0x1 << 12)){    
        pci_map_single(NULL, (unsigned long)(info->data_buff),info->cac_size, PCI_DMA_TODEVICE);
    }
    pci_map_single(NULL, (unsigned long)(info->drcmr_dat),0x20, PCI_DMA_TODEVICE);
...
//                info->state = STATE_READY;
	...
	wait_for_completion(&info->cmd_complete);
        ...
        if(info->cmd == NAND_CMD_READ0 || info->cmd == NAND_CMD_READOOB ){
            pci_unmap_single(NULL, (unsigned long)(info->data_buff_phys),info->cac_size, PCI_DMA_FROMDEVICE);
        }
        info->state = STATE_READY;
...
static int ls1a_nand_probe(struct pci_dev *pdev)
{
...
const char *part_probes[] = { "cmdlinepart", NULL };
	...
	mtd = kmalloc(sizeof(struct mtd_info) + sizeof(struct ls1a_nand_info),
			GFP_KERNEL);
	if (!mtd) {
	...
	memset(mtd, 0, sizeof(struct mtd_info) + sizeof(struct ls1a_nand_info));
	...
	this = &info->nand_chip;
	mtd->priv = info;
        info->drcmr_dat = pci_alloc_consistent(pdev, MAX_BUFF_SIZE,
				&info->drcmr_dat_phys);
        info->drcmr_dat = CAC_ADDR(info->drcmr_dat);
        info->dma_ask =(unsigned int ) pci_alloc_consistent(pdev, MAX_BUFF_SIZE,
				&info->dma_ask_phy);
...
//        printk("info->drcmr_dat===================0x%08x\n",info->drcmr_dat);
	...
	info->mmio_base = ioremap(reg_base, 0x40);
	if (info->mmio_base == NULL) {
		dev_err(&pdev->dev, "ioremap() failed\n");
		ret = -ENODEV;
		goto fail_free_res;
	}
	printk("info->mmio_base=0x%p\n", info->mmio_base);
	...
	ret = request_irq(irq, ls1a_nand_irq, SA_INTERRUPT, pdev->name, info);
	if (ret < 0) {
	...
	mtd->name = "nand-flash";
	num_partitions = parse_cmdline_partitions(mtd, &partitions, 0);
#endif
	if (num_partitions <= 0 )
...
fail_free_irq:
	free_irq(13, info);
fail_free_buf:
    	pci_free_consistent(pdev, info->data_buff_size,info->data_buff, info->data_buff_phys);
fail_free_io:
...
static int ls1a_nand_remove(struct pci_dev *pdev)
{
...
static int ls1a_nand_suspend(struct pci_dev *pdev)
{
	struct mtd_info *mtd = (struct mtd_info *)pci_get_drvdata(pdev);
	struct ls1a_nand_info *info = mtd->priv;
...
static int ls1a_nand_resume(struct pci_dev *pdev)
{
	struct mtd_info *mtd = (struct mtd_info *)pci_get_drvdata(pdev);
	struct ls1a_nand_info *info = mtd->priv;
    	*((volatile unsigned int *)(_NAND_BASE+0x18)) = 0x400;
...
static struct pci_device_id ls1a_nand_pci_tbl[] = {
    {PCI_DEVICE (LS1A_VENDOR_ID, LS1A_FLASH_DEVICE_ID),}, {0,} 
};

static struct pci_driver ls1a_nand_pci_driver = {
	.name		= "ls1a-nand",
	.id_table	= ls1a_nand_pci_tbl,
	.probe		= ls1a_nand_probe,
	.remove		= __devexit_p(ls1a_nand_remove),
#ifdef CONFIG_PM
...
static int __init ls1a_nand_init_module (void)
{
	/* when we're a module, we always print a version message,
	 * even if no 8139 board is found.
	 */
#ifdef MODULE
	printk (KERN_INFO "ls1a_nand_init_module\n");
#endif

	return pci_module_init (&ls1a_nand_pci_driver);
}


static void __exit ls1a_nand_cleanup_module (void)
{
	pci_unregister_driver (&ls1a_nand_pci_driver);
}
module_init(ls1a_nand_init_module);
module_exit(ls1a_nand_cleanup_module);
MODULE_LICENSE("GPL");

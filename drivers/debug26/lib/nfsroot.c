...
static __u32 servaddr  = 0;

/* Name of directory to mount */
static char nfs_path[NFS_MAXPATHLEN]  = { 0, };

/* NFS-related data */
static struct nfs_mount_data nfs_data  = { 0, };/* NFS mount info */
static int nfs_port  = 0;		/* Port to connect to for NFS */
static int mount_port  = 0;		/* Mount daemon port number */
...
static u32  root_nfs_parse_addr(char *name)
{
        u32 addr;
        int octets = 0;
        char *cp, *cq;
 ...
 *  command line.
 */
static int  nfs_root_setup(char *line)
{
 ...
 */
static int  root_nfs_getport(int program, int version, int proto)
{
 ...
 */
static void *  nfs_root_data(void)
{
	if (root_nfs_init() < 0
	 || root_nfs_ports() < 0
...
static int nr_mountnfs(int argc,char **argv)
{
	void *data;
int ret=0;
mm_segment_t    oldfs;
...
set_fs(KERNEL_DS);
	...
	nfs_root_setup(argv[1]);
	data = nfs_root_data();
	mknod("/dev/root", S_IFBLK|0600, MKDEV(UNNAMED_MAJOR, 1));
	if (data && mount("/dev/root",argv[2],"nfs",0,data) == 0)
		ret=1;
set_fs(oldfs);
	return ret;
}

mycmd_init(nfs,nr_mountnfs,"10.0.0.3:/mnt/sdb2/debian /mnt","nfs");

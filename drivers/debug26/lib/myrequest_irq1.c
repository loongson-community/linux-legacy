...
static int run=0;
static int stop=0;
static int keeprun=0;

static int mythread(void *unused)
{

	struct myirqlist *q;
	unsigned long long busy,idle;
	busy=0;
	idle=0;
run=1;
while(1)
...
if(!keeprun)schedule();
	...
	{
	int ret;
if(keeprun)	t0=read_c0_count();
	 ret=q->handler(0,q->dev_id,0);
if(keeprun)	t1=read_c0_count();
	//if(ret)busy+=(t1-t0);
	//else idle+=(t1-t0);
...
local_irq_disable();
#ifdef MY_USE_NAPI
if(local_softirq_pending()&(1<<NET_RX_SOFTIRQ))
net_rx_action(0);
#endif
}
}

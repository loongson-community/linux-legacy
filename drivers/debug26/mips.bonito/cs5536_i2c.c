#ifdef DEBUG26_CS5536_I2C

#define I2C_SINGLE 0 
#define I2C_BLOCK 1
#define I2C_SMB_BLOCK 2

static int	SMB_BASE_ADDR=0;
#define	SMB_SDA				0x00
#define	SMB_STS				0x01
#define	SMB_STS_SLVSTP		(1 << 7)
#define	SMB_STS_SDAST		(1 << 6)
#define	SMB_STS_BER		(1 << 5)
#define	SMB_STS_NEGACK		(1 << 4)
#define	SMB_STS_STASTR		(1 << 3)
#define	SMB_STS_NMATCH		(1 << 2)
#define	SMB_STS_MASTER		(1 << 1)
#define	SMB_STS_XMIT		(1 << 0)
#define	SMB_CTRL_STS			0x02
#define	SMB_CSTS_TGSTL		(1 << 5)
#define	SMB_CSTS_TSDA		(1 << 4)
#define	SMB_CSTS_GCMTCH		(1 << 3)
#define	SMB_CSTS_MATCH		(1 << 2)
#define	SMB_CSTS_BB		(1 << 1)
#define	SMB_CSTS_BUSY		(1 << 0)
#define	SMB_CTRL1			0x03
#define	SMB_CTRL1_STASTRE	(1 << 7)
#define	SMB_CTRL1_NMINTE	(1 << 6)
#define	SMB_CTRL1_GCMEN		(1 << 5)
#define	SMB_CTRL1_ACK		(1 << 4)
#define	SMB_CTRL1_RSVD		(1 << 3)
#define	SMB_CTRL1_INTEN		(1 << 2)
#define	SMB_CTRL1_STOP		(1 << 1)
#define	SMB_CTRL1_START		(1 << 0)
#define	SMB_ADDR			0x04
#define	SMB_ADDR_SAEN		(1 << 7)
#define	SMB_CTRL2			0x05
#define	SMB_ENABLE		(1 << 0)
#define	SMB_CTRL3			0x06


//p395
static int i2c_wait()
{
	char c;
	int i;
	udelay(1000);
	for(i=0;i<20;i++)
	{
	c = inb(SMB_BASE_ADDR|SMB_STS);
	if(c&SMB_STS_BER)return -1;
	if(c&SMB_STS_SDAST)return 0;
	udelay(100);
	}
	return -2;
}




static int i2c_read_single(int addr, int regNo,char *value)
{
	int i;
	unsigned char c;
	int j;
	outb(SMB_CTRL1_START,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	outb(addr&0xfe,SMB_BASE_ADDR);
	i2c_wait();
	outb(regNo,SMB_BASE_ADDR);
	i2c_wait();
	outb(SMB_CTRL1_ACK|SMB_CTRL1_START,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	outb(addr|1,SMB_BASE_ADDR);
	i2c_wait();
	*value=inb(SMB_BASE_ADDR);
	outb(2,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	//clear error
	//outb(0x10,SMB_BASE_ADDR|SMB_STS);
	c=inb(SMB_BASE_ADDR|SMB_STS);
	outb(0x30,SMB_BASE_ADDR|SMB_STS);
	return 0;
}

static int i2c_write_single(int addr, int regNo,char *value)
{
	int i;
	unsigned char c;
	int j;
	outb(SMB_CTRL1_START,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	outb(addr&0xfe,SMB_BASE_ADDR);
	i2c_wait();
	outb(regNo,SMB_BASE_ADDR);
	i2c_wait();
	outb(*value,SMB_BASE_ADDR);
	i2c_wait();
	outb(2,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	//clear error
	//outb(0x10,SMB_BASE_ADDR|SMB_STS);
	c=inb(SMB_BASE_ADDR|SMB_STS);
	outb(0x30,SMB_BASE_ADDR|SMB_STS);
	return 0;
}


static int i2c_read_smbblock(int addr,int regNo,unsigned char *buf,char len) {
	int i;
	unsigned char c,count;
	int j;

	outb(SMB_CTRL1_START,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	outb(addr&0xfe,SMB_BASE_ADDR);
	i2c_wait();
	outb(regNo,SMB_BASE_ADDR);
	i2c_wait();
	outb(SMB_CTRL1_START,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	outb(addr|1,SMB_BASE_ADDR);
	i2c_wait();
	count = inb((SMB_BASE_ADDR));
	udelay(4000);
	i2c_wait();
	for(i = 0; i < count+20; i++)
	{
	if(i2c_wait()<0){nr_printf("i=%d\n",i);break;}
	c=inb((SMB_BASE_ADDR));
	if(i<len)buf[i]=c;
	}
	i2c_wait();
	c=inb((SMB_BASE_ADDR));

	outb(2,SMB_BASE_ADDR|SMB_CTRL1);

	i2c_wait();
	//clear error
	c=inb(SMB_BASE_ADDR|SMB_STS);
	outb(0x30,SMB_BASE_ADDR|SMB_STS);
	return count;
}

static int i2c_write_smbblock(int addr,int regNo, char *buf,int len) {
	int i, j;
	unsigned char c;


	outb(SMB_CTRL1_START,SMB_BASE_ADDR|SMB_CTRL1);
	i2c_wait();
	outb(addr&0xfe,SMB_BASE_ADDR);
	i2c_wait();
	outb(regNo,SMB_BASE_ADDR);
	i2c_wait();

	outb(len,SMB_BASE_ADDR);
	i2c_wait();

	for(i=0;i<len;i++)
	{
	outb(buf[i],SMB_BASE_ADDR);
	i2c_wait();
	}

	outb(2,SMB_BASE_ADDR|SMB_CTRL1);

	i2c_wait();
	//clear error
	c=inb(SMB_BASE_ADDR|SMB_STS);
	outb(0x30,SMB_BASE_ADDR|SMB_STS);


	return len;
}


int tgt_i2cread(int type,unsigned char *addr,int addrlen,unsigned char reg,unsigned char *buf,int count)
{
int i;
memset(buf,-1,count);
switch(type)
{
case I2C_SINGLE:
	for(i=0;i<count;i++)
	{
	i2c_read_single(addr[0],reg+i,buf+i);
	}
break;
case I2C_SMB_BLOCK:
i2c_read_smbblock(addr[0],reg,buf,count);
default: return 0;break;
}
return count;
}

int tgt_i2cwrite(int type,unsigned char *addr,int addrlen,unsigned char reg,unsigned char *buf,int count)
{
int i;
switch(type)
{
case I2C_SINGLE:
	for(i=0;i<count;i++)
	{
	i2c_write_single(addr[0],reg,buf+i);
	}
break;
case I2C_SMB_BLOCK:
i2c_write_smbblock(addr[0],reg,buf,count);
break;
default:return -1;break;
}
return count;
}




static int i2c_88e1111_reg_read(int type,long long addr,union commondata *mydata)
{
char i2caddr[]={0x80};
switch(type)
{
case 2:
tgt_i2cread(I2C_SINGLE,i2caddr,1,addr,(void *)&mydata->data2+1,1);
tgt_i2cread(I2C_SINGLE,i2caddr,1,addr,&mydata->data2,1);
break;

default: return -1;break;
}
return 0;
}

static int i2c_88e1111_reg_write(int type,long long addr,union commondata *mydata)
{
char i2caddr[]={0x80};
switch(type)
{
case 2:
tgt_i2cwrite(I2C_SINGLE,i2caddr,1,addr,(void *)&mydata->data2+1,1);
tgt_i2cwrite(I2C_SINGLE,i2caddr,1,addr,&mydata->data2,1);
break;
default: return -1;break;
}
return 0;
}

static int i2cs_88e1111(int argc,char **argv)
{
	struct pci_dev *pcidev;
syscall1=i2c_88e1111_reg_read;
syscall2=i2c_88e1111_reg_write;
	pcidev=(void *)pci_find_slot(0,(14<<3)|0);
	pci_read_config_dword(pcidev,0x10,(void *)&SMB_BASE_ADDR);
	SMB_BASE_ADDR &= ~3;
	setenv("iomap","0x10","88e1111 i2c");
return 0;	
}

mycmd_init(i2cs_88e11,i2cs_88e1111,"","i2c for 88e1111");

static int syscall_i2c_type,syscall_i2c_addrlen;
static unsigned char syscall_i2c_addr[2];

static int i2c_read_syscall(int type,long long addr,union commondata *mydata)
{
char c;
switch(type)
{
case 1:
tgt_i2cread(syscall_i2c_type,syscall_i2c_addr,syscall_i2c_addrlen,addr,&mydata->data1,1);

break;

default: return -1;break;
}
return 0;
}

static int i2c_write_syscall(int type,long long addr,union commondata *mydata)
{
char c;
switch(type)
{
case 1:
tgt_i2cwrite(syscall_i2c_type,syscall_i2c_addr,syscall_i2c_addrlen,addr,&mydata->data1,1);

break;

default: return -1;break;
}
return 0;
return -1;
}
static int i2cs(int argc,char **argv)
{
	volatile int i;
	struct pci_dev *pcidev;

			if(argc<3)return -1;
	pcidev=(void *)pci_find_slot(0,(14<<3)|0);
	pci_read_config_dword(pcidev,0x10,(void *)&SMB_BASE_ADDR);
	SMB_BASE_ADDR &= ~3;
			syscall_i2c_type=nr_strtol(argv[1],0,0);
			syscall_i2c_addrlen=argc-2;
			for(i=2;i<argc;i++)syscall_i2c_addr[i-2]=nr_strtol(argv[i],0,0);
			 syscall1=(void*)i2c_read_syscall;
			 syscall2=(void*)i2c_write_syscall;
	syscall_addrtype=0;
nr_printf("type=%d,addrlen=%d,addr=0x%x\n",syscall_i2c_type,syscall_i2c_addrlen,syscall_i2c_addr[0]);

	return 0;
}
mycmd_init(i2cs,i2cs,"","i2cs to select i2c");
#endif

/*
 *  linux/drivers/video/ls3afb.c -- Simple frame buffer device
 *
...
static unsigned long smem_start, smem_len;
...
static char ls3afb_name[16] = "Simple FB";
static char *mode_option __initdata = 0;
static int inverse = 0;
static int grayscale = 0;

static struct fb_var_screeninfo ls3afb_default = {
...
int ls3afb_setup(char*);

static int ls3afb_get_fix(struct fb_fix_screeninfo *fix, int con,
		       struct fb_info *info);
static int ls3afb_get_var(struct fb_var_screeninfo *var, int con,
		       struct fb_info *info);
static int ls3afb_set_var(struct fb_var_screeninfo *var, int con,
		       struct fb_info *info);
static int ls3afb_pan_display(struct fb_var_screeninfo *var, int con,
			   struct fb_info *info);
static int ls3afb_get_cmap(struct fb_cmap *cmap, int kspc, int con,
			struct fb_info *info);
static int ls3afb_set_cmap(struct fb_cmap *cmap, int kspc, int con,
			struct fb_info *info);
...
int ls3afb_init(void);
static int ls3afbcon_switch(int con, struct fb_info *info);
static int ls3afbcon_updatevar(int con, struct fb_info *info);
static void ls3afbcon_blank(int blank, struct fb_info *info);
...
static void ls3afb_encode_fix(struct fb_fix_screeninfo *fix,
			   struct fb_var_screeninfo *var);
static void set_color_bitfields(struct fb_var_screeninfo *var);
static int ls3afb_getcolreg(u_int regno, u_int *red, u_int *green, u_int *blue,
                         u_int *transp, struct fb_info *info);
static int ls3afb_setcolreg(u_int regno, u_int red, u_int green, u_int blue,
                         u_int transp, struct fb_info *info);
static void do_install_cmap(int con, struct fb_info *info);


static struct fb_ops ls3afb_ops = {
    owner:          THIS_MODULE,
    fb_get_fix:     ls3afb_get_fix,
    fb_get_var:     ls3afb_get_var,
    fb_set_var:     ls3afb_set_var,
    fb_get_cmap:    ls3afb_get_cmap,
    fb_set_cmap:    ls3afb_set_cmap,
    fb_pan_display: ls3afb_pan_display,
};
			...
			struct fb_info *info)
{
	...
	return fb_get_cmap(cmap, kspc, ls3afb_getcolreg, info);
			...
			struct fb_info *info)
{
    ...
    if (con == currcon)			/* current console? */
	return fb_set_cmap(cmap, kspc, ls3afb_setcolreg, info);
    else
	fb_copy_cmap(cmap, &fb_display[con].cmap, kspc ? 0 : 1);
    return 0;
}

int __init ls3afb_setup(char *options)
{
    char *this_opt;
...
int __init ls3afb_init(void)
{
    struct fb_var_screeninfo var;

    strcpy(fb_info.modename, ls3afb_name);
    fb_info.changevar = NULL;
    fb_info.node = -1;
    fb_info.fbops = &ls3afb_ops;
    fb_info.disp = &disp;
    fb_info.switch_con = &ls3afbcon_switch;
    fb_info.updatevar = &ls3afbcon_updatevar;
    fb_info.blank = &ls3afbcon_blank;
...
static int ls3afbcon_switch(int con, struct fb_info *info)
{
	...
	fb_get_cmap(&fb_display[currcon].cmap, 1, ls3afb_getcolreg, info);
			   ...
			   struct fb_var_screeninfo *var)
{
    ...
    strcpy(fix->id, ls3afb_name);
    fix->smem_start = smem_start;
...
static void do_install_cmap(int con, struct fb_info *info)
{
	...
	fb_set_cmap(&fb_display[con].cmap, 1, ls3afb_setcolreg, info);
    else
	fb_set_cmap(fb_default_cmap(1<<fb_display[con].var.bits_per_pixel), 1,
		    ls3afb_setcolreg, info);
}
//----------------------------------------------------
...
static int __devinit vgx_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	*/
	...
	videomemorysize = smem_len = pci_resource_len(dev,0);
...
static struct pci_device_id vgx_devices[] = {
	{PCI_ANY_ID, PCI_ANY_ID, PCI_ANY_ID, PCI_ANY_ID, PCI_BASE_CLASS_DISPLAY << 16,
          0xff0000, 0 },
	{0}
};

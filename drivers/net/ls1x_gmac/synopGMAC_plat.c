...
void *plat_alloc_consistent_dmaable_memory(void *dev, u32 size, dma_addr_t *addr) 
{
#ifdef CONFIG_GMAC_COHERENT
 return (dma_alloc_noncoherent(dev,size,addr,GFP_ATOMIC));
#else
 return (pci_alloc_consistent(NULL, size,addr));
#endif
}
...
void plat_free_consistent_dmaable_memory(void *dev, u32 size, void * addr,u32 dma_addr) 
{
#ifdef CONFIG_GMAC_COHERENT
 dma_free_noncoherent (dev,size,addr,dma_addr);
#else
 pci_free_consistent (NULL, size,addr,dma_addr);
#endif
 return;
}

...
static int probe (struct pci_dev *pdev, const struct pci_device_id *my_pci_id) 
{
  ...
  TR ("Remapped address = %p\n",  synopGMACMappedAddr);
    ...
    if (check_mem_region ( (void *)synopGMACMappedAddr, synopGMACMappedAddrSize))
    {
    ...
    request_mem_region ((void *) synopGMACMappedAddr, synopGMACMappedAddrSize,"synopGMACmemory");
	...
	if((synopGMACadapter=synopGMAC_init_network_interface(NULL, synopGMACMappedAddr,synopGMACMappedAddrSize,pdev->irq)))
...
struct net_device *netdev=adapter->synopGMACnetdev;
    ...
    if (adapter->synopGMACMappedAddr)
    {
      ...
      release_mem_region ((void *) adapter->synopGMACMappedAddr, adapter->synopGMACMappedAddrSize);
    }

...
static int ls1x_gmac_suspend(struct platform_device *pdev, pm_message_t state)
{
	synopGMACPciNetworkAdapter *adapter=platform_get_drvdata(pdev);
...
static int ls1x_gmac_resume(struct platform_device *pdev)
{
	synopGMACPciNetworkAdapter *adapter=platform_get_drvdata(pdev);
	synopGMACdevice * gmacdev = adapter->synopGMACdev;
	struct net_device *ndev = adapter->synopGMACnetdev;
...
#else
#define ls1x_gmac_suspend	NULL
#define ls1x_gmac_resume	NULL
#endif
static struct platform_driver ls1x_gmac_driver = {
	.probe			= init_one_platform,
	.remove			= remove_one_platform,
	.suspend		= ls1x_gmac_suspend,
	.resume			= ls1x_gmac_resume,
	.driver		= {
		.name	= "ls1x-gmac",
		.owner	= THIS_MODULE,
	},
...
static int __init ls1x_gmac_init(void)
{
	platform_driver_register(&ls1x_gmac_driver);
	return 0;
}


module_init(ls1x_gmac_init);

static void __exit ls1x_gmac_exit(void)
{
	platform_driver_unregister(&ls1x_gmac_driver);
}

module_exit(ls1x_gmac_exit);

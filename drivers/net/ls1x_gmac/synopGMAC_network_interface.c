...
s32 synopGMAC_linux_open(struct net_device *netdev)
{
	...
	if(request_irq (adapter->irq, synopGMAC_intr_handler, SA_SHIRQ, netdev->name,netdev)){
 ...
 * \return Returns 0 on success and Error code on failure.
 */
void * __init synopGMAC_init_network_interface(void *dev,u8* synopGMACMappedAddr,u32 synopGMACMappedAddrSize,int irq)
{
	s32 err;
	struct net_device *netdev;

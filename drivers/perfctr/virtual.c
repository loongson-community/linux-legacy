/* $Id: virtual.c,v 1.2 2004/06/07 02:27:50 fxzhang Exp $
...
static void vperfctr_ihandler(unsigned long pc)
{
...
#if 0	
	if( !perfctr ) {
		printk(KERN_DEBUG "%s: BUG! pid %d has no vperfctr\n",
		       __FUNCTION__, tsk->pid);
		return;
	}
	if( !perfctr_cstatus_has_ictrs(perfctr->cpu_state.cstatus) ) {
		printk(KERN_DEBUG "%s: BUG! vperfctr has cstatus %#x (pid %d, comm %s)\n",
		       __FUNCTION__, perfctr->cpu_state.cstatus, tsk->pid, tsk->comm);
		return;
	}
#else
	/* these are normal when we are sharing interrupts with others */
	if( !perfctr || !perfctr_cstatus_has_ictrs(perfctr->cpu_state.cstatus)) {
		return;
	}
#endif
	...
	if( !pmc_mask ) {
		...
		printk(KERN_DEBUG "%s: BUG! pid %d has unidentifiable overflow source\n",
		       __FUNCTION__, tsk->pid);
		return;
	}
...
{
	/*
	prom_printf("sus:perfctr=%lx\n",(unsigned long)perfctr);
	return;
	*/
	if( IS_RUNNING(perfctr) )
		vperfctr_suspend(perfctr);
...
#else
	    (pgprot_val(vma->vm_page_prot) & _PAGE_WRITE) ||
#endif

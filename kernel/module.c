...
void *find_symbol(struct module *mod,char *name)
{ 
	int i;
	if(mod)
	{
...
#ifdef CONFIG_RTHAL
asmlinkage int (*myrt_printk)(const char *fmt, ...) = printk ;
void **myrt_current;
void *myrt_linux_task;
#endif
...
sys_init_module(const char *name_user, struct module *mod_user)
{
	...
	{
	 void *p;
		if(p=find_symbol(mod,"rt_printk"))
		 myrt_printk = p;
		if(p=find_symbol(mod,"rt_linux_task"))
		 myrt_linux_task = p;
		if(p=find_symbol(mod,"rt_current"))
		 myrt_current = p;
	}
#endif

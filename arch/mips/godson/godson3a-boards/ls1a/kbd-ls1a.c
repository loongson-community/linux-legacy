...
#define AUX_IRQ 11
#define KBD_ADDR (int)0xb2e60000
#undef KBD_STATUS_REG
#undef KBD_CNTL_REG
#undef KBD_DATA_REG

#define KBD_STATUS_REG		(int)(KBD_ADDR+0x10)	/* Status register (R) */
#define KBD_CNTL_REG		(int)(KBD_ADDR+0x10)	/* Controller command register (W) */
#define KBD_DATA_REG		(int)(KBD_ADDR+0x0)	/* Keyboard data register (R/W) */
...
static void std_kbd_write_output(unsigned char val)
{
	...
	do {
		status = readb(KBD_CNTL_REG);
	} while (status & KBD_STAT_IBF);
	writeb(val, KBD_DATA_REG);
}
...
static void std_kbd_write_command(unsigned char val)
{
	...
	writeb(val, KBD_CNTL_REG);
}

static unsigned char std_kbd_read_status(void)
{
	return readb(KBD_STATUS_REG);
}

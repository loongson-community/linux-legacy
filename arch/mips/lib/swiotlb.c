...
#define virt_to_baddr(ptr) bus_to_baddr(0, virt_to_bus(ptr))
#define baddr_to_virt(dmaaddr) bus_to_virt(baddr_to_bus(0, dmaaddr))
...
#define SG_ENT_PHYS_ADDRESS(SG)	virt_to_baddr(SG_ENT_VIRT_ADDRESS(SG))
...
static int
address_needs_mapping(struct pci_dev *hwdev, dma_addr_t addr)
{
#ifdef CONFIG_LS2F1A_SWIOTLB
    if(addr >= 0x10000000 && addr <= 0x1bffffff)
        return 0;
    else
	return 1;
#else
...
swiotlb_init (void)
{
	 ...
	 */
#ifdef CONFIG_LS2F1A_SWIOTLB
	io_tlb_start = __alloc_bootmem((io_tlb_nslabs * (1 << IO_TLB_SHIFT)), SMP_CACHE_BYTES, 0x4000000);
#else
...
swiotlb_alloc_consistent (struct pci_dev *hwdev, size_t size, dma_addr_t *dma_handle)
{
	...
	memset(ret, 0, size);
	pci_addr = virt_to_baddr(ret);
	if(address_needs_mapping(hwdev, pci_addr))
...
{
	unsigned long pci_addr = virt_to_baddr(ptr);
	 ...
	 * get a bounce buffer:
	 */
	pci_addr = virt_to_baddr(map_single(hwdev, ptr, size, direction));
...
{
	char *dma_addr = baddr_to_virt(pci_addr);
...
swiotlb_map_sg (struct pci_dev *hwdev, struct scatterlist *sg, int nelems, int direction)
{
	...
	for (i = 0; i < nelems; i++, sg++) {
		...
		unsigned long phys_address = virt_to_baddr(virt_address);

		sg->dma_length = sg->length;
		if (address_needs_mapping(hwdev, phys_address))
			sg->dma_address = virt_to_baddr(map_single(hwdev,
			                                          virt_address,
			                                          sg->length,
			                                          direction));
		else
...
swiotlb_unmap_sg (struct pci_dev *hwdev, struct scatterlist *sg, int nelems, int direction)
{
	...
	for (i = 0; i < nelems; i++, sg++)
	{
		...
		if(!sg->dma_length)
		continue;
		if (sg->dma_address != virt_to_baddr(SG_ENT_VIRT_ADDRESS(sg))) {
			unmap_single(hwdev, baddr_to_virt(sg->dma_address),
...
swiotlb_sync_sg (struct pci_dev *hwdev, struct scatterlist *sg, int nelems, int direction)
{
	...
	for (i = 0; i < nelems; i++, sg++)
		if (sg->dma_address != virt_to_baddr(SG_ENT_VIRT_ADDRESS(sg)))
			sync_single(hwdev, baddr_to_virt(sg->dma_address),
			            sg->dma_length, direction);
}

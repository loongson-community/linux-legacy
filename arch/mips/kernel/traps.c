...
unsigned int myintr_error;
EXPORT_SYMBOL(myintr_error);
...
{
	struct task_struct *task,*fptask;
	union mips_fpu_union fpu;

	if (!user_mode(regs))
	{
		task = &init_task;
		fptask = list_entry(&fpu,struct task_struct,thread.fpu);
	}
	else
		fptask = task = current;
	...
	if (fcr31 & FPU_CSR_UNI_X) {
		...
		save_fp(fptask); 
		oldds = get_fs();
		if((regs->cp0_epc&0xc0000000) == 0xc0000000)
		set_fs(KERNEL_DS);

		/* Run the emulator */
		sig = fpu_emulator_cop1Handler (0, regs,
			&fptask->thread.fpu.soft);
		 ...
		 */
		fptask->thread.fpu.soft.sr &= ~FPU_CSR_ALL_X;
...
#else
	if (!user_mode(regs))
	printk("<0>do_cpu invoked from kernel context!epc=0x%x\n",regs->cp0_epc);
#endif
...
void __init per_cpu_trap_init(void)
{
	...
	clear_c0_status(ST0_CU3|ST0_CU2|ST0_CU1|ST0_BEV|ST0_SX|ST0_UX);

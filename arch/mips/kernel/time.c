...
/* add for performance counter support */
unsigned long long perf_counterloop=0;
unsigned int perf_lastvalue=0;
...
void timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if(perf_lastvalue>counter) perf_counterloop++;
	perf_lastvalue=counter;
#endif
	mips_timer_ack();

...
static inline int cpu_probe_release(void)
{
	unsigned long cputype_flag = 0;
	 __asm__ volatile ( ".set mips64;dli     $2, 0x90000efdfb000000\r\n"
                " lw      %0, 0x5c($2)\r\n"
                ".set     mips0\r\n"
                :"=r"(cputype_flag)::"$2");
...
static __attribute__((noinline)) void cpu_probe_legacy(struct cpuinfo_mips *c)
{
	...
	case PRID_IMP_GODSON232:
		c->cputype = CPU_GODSON232;
		c->isa_level = MIPS_CPU_ISA_II;
		c->options = MIPS_CPU_TLB | MIPS_CPU_4KEX | 
			     MIPS_CPU_4KTLB | MIPS_CPU_COUNTER | 
			     MIPS_CPU_FPU | MIPS_CPU_LLSC |
	...
	case PRID_IMP_GODSON2:
		c->isa_level = MIPS_CPU_ISA_III;
		c->options = MIPS_CPU_TLB | MIPS_CPU_4KEX | 
			     MIPS_CPU_4KTLB | MIPS_CPU_COUNTER | 
			     MIPS_CPU_FPU | MIPS_CPU_LLSC |
#ifdef CONFIG_GODSON_WATCH
			     MIPS_CPU_WATCH |
#endif
			     MIPS_CPU_32FPR;
		c->tlbsize = 64;
		...
		cpu_name[CPU_GODSON2] = "ICT Loongson-2H";
		}
#else
		if ((c->processor_id & 0xf) == (PRID_IMP_LOONGSON3A & 0xf)) {
			c->cputype = CPU_LOONGSON3;
			if(strstr(eboard->name,"2GQ"))
				cpu_name[CPU_GODSON2] = "ICT Loongson-2GQ";
			else{
				if(cpu_probe_release())
					cpu_name[CPU_GODSON2] = "ICT Loongson-3A5";
				else
					cpu_name[CPU_GODSON2] = "ICT Loongson-3A";
			}
		}
#endif
		else if ((c->processor_id & 0xf) == (PRID_IMP_LOONGSON3B & 0xf)) {
			c->cputype = CPU_LOONGSON3;
			cpu_name[CPU_GODSON2] = "ICT Loongson-3B";
		}
		else if ((c->processor_id & 0xf) == (PRID_IMP_LOONGSON3B1500 & 0xf)) {
			c->cputype = CPU_LOONGSON3;
			cpu_name[CPU_GODSON2] = "ICT Loongson-3B1500";
		}
		else {
			c->cputype = CPU_GODSON2;
			cpu_name[CPU_GODSON2] = "ICT Loongson-2";
		}
		break;
	default:
		c->cputype = CPU_UNKNOWN;

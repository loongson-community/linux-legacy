...
static void sm501_write_codec(struct sm501_audio *s, u8 addr, u16 data)
{
	     ...
	     (unsigned int) data << 4) && (addr != AC97_RESET))
	{
		 ...
		 */
		if (addr != 0x2a)  /* 2A register bit0 means "Enable VRA", some codec doesn't support */
			printk(KERN_ERR"%s failed: addr=%02x data=%04x read_back=%04x\n",
				__func__, addr, data, read_back);
	}
	else
	{	
		dprintk("%s ok: addr=%02x data=%04x\n",
		__func__, addr, data);
	}
}
